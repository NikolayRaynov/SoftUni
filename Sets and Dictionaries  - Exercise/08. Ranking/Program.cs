...
namespace _08._Ranking
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<string, Dictionary<string, int>> bestIntern = users.OrderByDescending
                (x => x.Value.Values.Sum()).ToDictionary
                (x => x.Key, x => x.Value);

            Console.WriteLine($"Best candidate is {bestIntern.First().Key} with total " +
                $"{bestIntern.First().Value.Values.Sum()} points.");
